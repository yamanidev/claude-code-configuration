...
4. **Choose the file path.** Default to `mktemp -t handoff-XXXXXX.md` for a unique, collision-free path. If the user named a specific path, use that — and read the file first if it may already exist.

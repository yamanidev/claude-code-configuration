...
name: draft-logo
description: >-
  Workflow for drafting a logo mark for a side project or utility, producing a self-contained HTML gallery of 4–6 concept directions per round — each rendered in SVG at large, medium, and favicon sizes on both light and dark backgrounds, so the user can open the gallery in a browser, pick favorites by pick code, and converge through subsequent rounds. Output lives in the project's `logos/` directory. Forces a written brief before any SVG is drawn, requires each concept to commit to its SVG technique (Beziers, gradients, filters, masks — whatever the idea calls for), names concepts by their visual idea and tags each with a short pick code for effortless reference, and in later rounds variates within the picked directions instead of restarting from scratch. NOT for full brand identity work (palette systems, type pairings, voice guides, marketing copy) and NOT for editing application code to install the logo into a UI (use /ship for that). MANUAL INVOCATION ONLY: invoke this skill ONLY when the user explicitly types /draft-logo. Do not auto-invoke when the user mentions logos, branding, icons, SVG, or design — handle those in normal conversation unless the user explicitly opts in with /draft-logo.
---
